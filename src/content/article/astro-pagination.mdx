...
const getPageUrl = (pageNumber) => {
  return pageNumber === 1 ? page.url.first : `/blog/${pageNumber}`;
}
---
...
<div class="table-scroll">
  <table>
    ...
    <tbody>
      ...
      <tr>
        <td>`page.url.prev`</td>
        <td>前のページのURL。もし現在表示中のページが1ページ目なら、`undefined`になる。</td>
      </tr>
      <tr>
        <td>`page.url.prev`</td>
...
`pageNumber`が`1`の場合は`page.url.first`を返し、それ以外は`/blog/数字`の形式でURLを生成します。

`blog`の部分は自身のページのパスに合わせて変更してください。
